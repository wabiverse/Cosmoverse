...
private func forceCast<U>(_ from: some Any, to _: U.Type) -> U
{
  from as! U
}
...
public extension RealmCollection where Element: ObjectBase
{
  ...
  {
    sectioned(sortDescriptors: [.init(keyPath: _name(for: keyPath), ascending: ascending)])
    {
      $0[keyPath: keyPath]
    }
  }
  ...
  {
    guard let sortDescriptor = sortDescriptors.first
    else
    {
  ...
  func sectioned<Key: _Persistable>(by block: @escaping ((Element) -> Key),
                                    sortDescriptors: [SortDescriptor]) -> SectionedResults<Key, Element> where Element: ObjectBase
  {
    sectioned(sortDescriptors: sortDescriptors, block)
  }
}
...
public extension RealmCollection where Element: KeypathSortable
{
  ...
  {
    distinct(by: keyPaths.map(_name(for:)))
  }
}
...
public extension RealmCollection
{
  ...
  func index(matching predicateFormat: String, _ args: Any...) -> Int?
  {
    index(matching: NSPredicate(format: predicateFormat, argumentArray: unwrapOptionals(in: args)))
  }
  ...
  func filter(_ predicateFormat: String, _ args: Any...) -> Results<Element>
  {
    filter(NSPredicate(format: predicateFormat, argumentArray: unwrapOptionals(in: args)))
  }
}
...
@frozen public struct AnyRealmCollection<Element: RealmCollectionValue>: RealmCollectionImpl
{
  ...
  {
    RLMDescriptionWithMaxDepth("AnyRealmCollection", collection, RLMDescriptionMaxDepth)
  }
...
public struct ProjectedCollection<Element>: RandomAccessCollection, CustomStringConvertible, ThreadConfined where Element: RealmCollectionValue
{
  ...
  {
    backingCollection.map { $0[keyPath: keyPath] as! Element }.firstIndex(of: object)
  }
  ...
  {
    guard let backingCollection = backingCollection.thaw()
    else
    {
      return nil
    }
