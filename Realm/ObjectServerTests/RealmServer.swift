...
import XCTest
  ...
  fileprivate extension URLSession
  {
    ...
    {
      URLSession(configuration: .default, delegate: nil, delegateQueue: OperationQueue()).dataTask(with: request)
    ...
    func resultDataTask(with request: URLRequest) -> Result<Data, Error>
    {
      ...
      guard case .success = group.wait(timeout: .now() + 10)
      else
      {
  ...
  private func bsonType(_ type: PropertyType) -> String
  {
    switch type
    {
      ...
      case .string: "string"
      case .linkingObjects: "linkingObjects"
    }
  }
  ...
  private extension Property
  {
    ...
    {
      let type: String = if self.type == .object
      {
        bsonType(objectSchema.primaryKeyProperty!.type)
      }
      else
      {
        bsonType(self.type)
  ...
  class AdminSession
  {
    ...
    struct AdminEndpoint
    {
      ...
      {
        AdminEndpoint(accessToken: accessToken,
                      groupId: groupId,
        ...
        URLSession(configuration: URLSessionConfiguration.default,
          ...
          { result in
            ...
            { data in
              Result
              {
                !data.isEmpty ? try JSONSerialization.jsonObject(with: data) : nil
              }
            })
          }
      }
      ...
      private func request(httpMethod: String, data: Any? = nil) -> Result<Any?, Error>
      {
        ...
        guard case .success = group.wait(timeout: .now() + 60)
        else
        {
  ...
  class Admin
  {
    ...
    func login() throws -> AdminSession
    {
      ...
      return try URLSession(configuration: .default, delegate: nil, delegateQueue: OperationQueue())
        ...
        .flatMap
        { (accessToken: String) -> Result<AdminSession, Error> in
  ...
  public class RealmServer: NSObject
  {
    ...
    private func launchServerProcess() throws
    {
      ...
      pipe.fileHandleForReading.readabilityHandler = { file in
        guard !file.availableData.isEmpty,
        ...
        for part in available
        {
          ...
          {
            guard logLevel == .info || logLevel == .warn
            else
            {
              return
            }
    ...
    private func waitForServerToStart()
    {
      ...
      @Sendable func pingServer(_ tries: Int = 0)
      {
        ...
        session.dataTask(with: URL(string: "http://localhost:9090/api/admin/v3.0/groups/groupId/apps/appId")!)
        { _, _, error in
          if error != nil
          {
      ...
      pingServer()
      guard case .success = group.wait(timeout: .now() + 20)
      else
      {
    ...
    func createApp(syncMode: SyncMode, types: [ObjectBase.Type], persistent: Bool) throws -> AppId
    {
      ...
      app.authProviders.get(on: group)
      { authProviders in
        do
        {
          guard let authProviders = try authProviders.get() as? [[String: Any]]
          else
          {
            return XCTFail("Bad formatting for authProviders")
          }
          guard let provider = authProviders.first(where: { $0["type"] as? String == "api-key" })
          else
          {
      ...
      guard let serviceId = try (serviceResponse.get() as? [String: Any])?["_id"] as? String
      else
      {
        throw URLError(.badServerResponse)
      }
      ...
      let partitionKeyType: String? = if case let .pbs(bsonType) = syncMode
      {
        bsonType
      }
      else
    ...
    {
      try createApp(syncMode: .flx(fields), types: types, persistent: persistent)
    }

    @objc public func createApp(partitionKeyType: String = "string", types: [ObjectBase.Type], persistent: Bool = false) throws -> AppId
    {
      try createApp(syncMode: .pbs(partitionKeyType), types: types, persistent: persistent)
    }
    ...
    public func retrieveAppServerId(_ clientAppId: String) throws -> String
    {
      ...
      guard let appsList = appsListInfo as? [[String: Any]]
      else
      {
        throw URLError(.badServerResponse)
      }
      ...
      let app = appsList.first(where: {
        guard let clientId = $0["client_app_id"] as? String
        else
        {
          return false
        }
      ...
      guard let appId = app?["_id"] as? String
      else
      {
        throw URLError(.badServerResponse)
      }
    ...
    public func retrieveSyncServiceId(appServerId: String) throws -> String
    {
      ...
      // Get all services
      guard let syncServices = try app.services.get().get() as? [[String: Any]]
      else
      {
    ...
    public func isSyncEnabled(flexibleSync: Bool = false, appServerId: String, syncServiceId: String) throws -> Bool
    {
      ...
      guard let syncInfo = response?[configOption] as? [String: Any]
      else
      {
        return false
      }
    ...
    public func enableSync(flexibleSync: Bool = false, appServerId: String, syncServiceId: String, syncServiceConfiguration: [String: Any]) -> Result<Any?, Error>
    {
      ...
      let app = session.apps[appServerId]
      guard var syncInfo = syncConfig[configOption] as? [String: Any]
      else
      {
        return .failure(URLError(.unknown))
      }
    ...
    public func retrieveSchemaProperties(_ appId: String, className: String, _ completion: @escaping (Result<[String], Error>) -> Void)
    {
      ...
      completion(.success(schemaProperties.compactMap(\.key)))
    }
    ...
    public func waitForSync(appServerId: String, expectedCount: Int) throws
    {
      ...
      while true
      {
        let complete = try session.apps[appServerId].sync.progress.get()
          .map
          { resp in
            guard let resp = resp as? [String: Any?] else { return false }
            guard let progress = resp["progress"] else { return false }
            guard let progress = progress as? [String: Any?] else { return false }
            let values = progress.compactMapValues { $0 as? [String: Any?] }
  ...
  @Sendable private func failOnError(_ result: Result<some Any, Error>)
  {
