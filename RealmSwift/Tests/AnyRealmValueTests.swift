...
extension SwiftStringObject: AnyValueFactory
{
  ...
  {
    [SwiftStringObject(value: ["a"]), SwiftStringObject(value: ["b"]), SwiftStringObject(value: ["c"])]
  }
...
class AnyRealmValueObjectTests: TestCase
{
  ...
  func testDynamicObjectAccessor()
  {
    ...
    XCTAssertFalse(realm.schema.objectSchema.map(\.className).contains("SwiftStringObject"))
    guard let object = realm.objects(SwiftObject.self).first
    else
    {
      return XCTFail("SwiftObject does not exist")
    }
    guard let dynamicObject = object.anyCol.value.dynamicObject
    else
    {
