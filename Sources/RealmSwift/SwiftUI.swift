...
{
  value.realm == nil ? value : value.thaw() ?? value
}
...
{
  guard let value = value.isFrozen ? value.thaw() : value
  else
  {
...
@available(iOS 13.0, macOS 10.15, tvOS 13.0, watchOS 6.0, visionOS 1.0, *)
@objc(RLMSwiftUIKVO) final class SwiftUIKVO: NSObject
{
...
@propertyWrapper public struct ObservedResults<ResultType>: DynamicProperty, BoundCollection where ResultType: _ObservedResultsValue & RealmFetchable & KeypathSortable & Identifiable
{
  ...
  fileprivate func searchText(_ text: String, on keyPath: KeyPath<some ObjectBase, String>)
  {
...
@propertyWrapper public struct ObservedSectionedResults<Key: _Persistable & Hashable, ResultType>: DynamicProperty, BoundCollection where ResultType: _ObservedResultsValue & RealmFetchable & KeypathSortable & Identifiable
{
  ...
  private class Storage: ObservableResultsStorage<SectionedResults<Key, ResultType>>
  {
    override func updateValue()
    {
      ...
      if let keyPathString, sortDescriptors.isEmpty
      {
...
@propertyWrapper public struct ObservedRealmObject<ObjectType>: DynamicProperty where ObjectType: RealmSubscribable & ThreadConfined & ObservableObject & Equatable
{
  ...
  {
    Wrapper(wrappedValue: storage.value.isInvalidated ? defaultValue : storage.value)
  }
...
public extension EnvironmentValues
{
  ...
  var realm: Realm
  {
    get
    {
      try! Realm(configuration: self[RealmEnvironmentKey.self])
    }
    set
    {
  ...
  var partitionValue: PartitionValue?
  {
    get
    {
      self[PartitionValueEnvironmentKey.self]
    }
    set
    {
...
private class ObservableAsyncOpenStorage: ObservableObject
{
  ...
  private func asyncOpenForUser(_ user: User)
  {
    ...
    if var configuration
    {
      ...
      guard configuration.syncConfiguration != nil
      else
      {
  ...
  class func configureApp(appId: String? = nil, timeout: UInt? = nil) -> App
  {
    ...
    else
    {
      ...
      guard let cachedApp = cachedApps.first
      else
      {
...
public extension View
{
  ...
  /**
    ...
    */
  func searchable<T: ObjectBase>(text: Binding<String>, collection: ObservedResults<T>, keyPath: KeyPath<T, String>,
                                 placement: SearchFieldPlacement = .automatic, prompt: some StringProtocol) -> some View
  {
    filterCollection(collection, for: text.wrappedValue, on: keyPath)
    return searchable(text: text, placement: placement, prompt: prompt)
  }
  ...
  /**
    ...
    */
  func searchable<T: ObjectBase>(text: Binding<String>, collection: ObservedResults<T>, keyPath: KeyPath<T, String>,
                                 placement: SearchFieldPlacement = .automatic, prompt: Text? = nil, @ViewBuilder suggestions: () -> some View)
    -> some View
  {
    filterCollection(collection, for: text.wrappedValue, on: keyPath)
  ...
  /**
    ...
    */
  func searchable<T: ObjectBase>(text: Binding<String>, collection: ObservedResults<T>, keyPath: KeyPath<T, String>,
                                 placement: SearchFieldPlacement = .automatic, prompt: LocalizedStringKey, @ViewBuilder suggestions: () -> some View)
    -> some View
  {
    filterCollection(collection, for: text.wrappedValue, on: keyPath)
    return searchable(text: text,
  ...
  /**
    ...
    */
  func searchable<T: ObjectBase>(text: Binding<String>, collection: ObservedResults<T>, keyPath: KeyPath<T, String>,
                                 placement: SearchFieldPlacement = .automatic, prompt: some StringProtocol, @ViewBuilder suggestions: () -> some View)
    -> some View
  {
    filterCollection(collection, for: text.wrappedValue, on: keyPath)
    return searchable(text: text,
  ...
  /**
   ...
   - parameter prompt: A `Text` representing the prompt of the search field
               which provides users with guidance on what to search for.
    */
  func searchable<T: ObjectBase>(text: Binding<String>, collection: ObservedSectionedResults<some Any, T>, keyPath: KeyPath<T, String>,
                                 placement: SearchFieldPlacement = .automatic, prompt: Text? = nil) -> some View
  {
    filterCollection(collection, for: text.wrappedValue, on: keyPath)
    return searchable(text: text, placement: placement, prompt: prompt)
  ...
  /**
   ...
   - parameter prompt: The key for the localized prompt of the search field
               which provides users with guidance on what to search for.
    */
  func searchable<T: ObjectBase>(text: Binding<String>, collection: ObservedSectionedResults<some Any, T>,
                                 keyPath: KeyPath<T, String>, placement: SearchFieldPlacement = .automatic,
                                 prompt: LocalizedStringKey) -> some View
  {
    filterCollection(collection, for: text.wrappedValue, on: keyPath)
  ...
  private func filterCollection<T: ObjectBase>(_ collection: ObservedSectionedResults<some Any, T>, for text: String, on keyPath: KeyPath<T, String>)
  {
    assumeOnMainActorExecutor
    {
      collection.searchText(text, on: keyPath)
