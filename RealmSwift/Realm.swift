...
@frozen public struct Realm
{
  ...
  public var schema: Schema { Schema(rlmRealm.schema) }

  /// The `Configuration` value that was used to create the `Realm` instance.
  public var configuration: Configuration { Configuration.fromRLMRealmConfiguration(rlmRealm.configuration) }

  /// Indicates if the Realm contains any objects.
  public var isEmpty: Bool { rlmRealm.isEmpty }
  ...
  {
    AsyncOpenTask(rlmTask: RLMRealm.asyncOpen(with: configuration.rlmConfiguration, callbackQueue: callbackQueue, callback: { rlmRealm, error in
  ...
  {
    RealmPublishers.AsyncOpenPublisher(configuration: configuration)
  }
  ...
  @frozen public struct AsyncOpenTask
  {
    ...
    {
      rlmTask.addSyncProgressNotification(on: queue)
      { progress in
  ...
  {
    rlmRealm.commitAsyncWriteTransaction(onComplete, allowGrouping: allowGrouping)
  }
  ...
  {
    rlmRealm.isPerformingAsynchronousWriteOperations
  }
  ...
  /**
   ...
   :nodoc:
   */
  public func delete(_ objects: List<some ObjectBase>)
  {
  ...
  /**
   ...
   :nodoc:
   */
  public func delete(_ map: Map<some _MapKey, (some ObjectBase)?>)
  {
  ...
  /**
   ...
   :nodoc:
   */
  public func delete(_ objects: Results<some ObjectBase>)
  {
  ...
  {
    Results(RLMGetObjects(rlmRealm, type.className(), nil))
  }
  ...
  {
    Results<DynamicObject>(RLMGetObjects(rlmRealm, typeName, nil))
  }
   ...
   */
  public func object<Element: Object>(ofType type: Element.Type, forPrimaryKey key: some Any) -> Element?
  {
    unsafeBitCast(RLMGetObject(rlmRealm, (type as Object.Type).className(),
  ...
  {
    unsafeBitCast(RLMGetObject(rlmRealm, typeName, key) as! RLMObjectBase?, to: DynamicObject?.self)
  }
  ...
  {
    isFrozen ? self : Realm(rlmRealm.freeze())
  }
  ...
  {
    isFrozen ? Realm(rlmRealm.thaw()) : self
  }
  ...
  {
    RLMRealm.fileExists(for: config.rlmConfiguration)
  }
  ...
  {
    try RLMRealm.deleteFiles(for: config.rlmConfiguration)
  }
...
public extension Realm
{
  ...
  {
    SyncSubscriptionSet(rlmRealm.subscriptions)
  }
}
...
public extension Realm
{
  /**
   ...
   - parameter value:  The value used to populate the object.
   */
  func create(_ type: (some AsymmetricObject).Type, value: Any = [String: Any]())
  {
                             ...
                             _ downloadBeforeOpen: Realm.OpenBehavior) -> Bool
{
  switch downloadBeforeOpen
  {
    case .never:
      false
    case .once:
      !Realm.fileExists(for: configuration)
    case .always:
      true
  }
}
...
public extension Realm
{
  ...
  {
    guard let actor = rlmRealm.actor as? Actor
    else
    {
      fatalError("asyncWrite() can only be called on main thread or actor-isolated Realms")
    }
    return try await withoutActuallyEscaping(block)
    { block in
...
@available(macOS 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
private func openRealm(configuration: Realm.Configuration,
                       scheduler _: RLMScheduler,
                       actor: isolated some Actor,
                       downloadBeforeOpen: Realm.OpenBehavior) async throws -> Unchecked<RLMRealm>
{
