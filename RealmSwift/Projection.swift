...
public struct Projected<T: ObjectBase, Value>: AnyProjected
{
  ...
  ) -> Value
  {
    ...
    set
    {
      guard let keyPath = observed[keyPath: storageKeyPath].projectedKeyPath as? WritableKeyPath<T, Value>
      else
      {
...
public extension ProjectionObservable
{
               ...
               _ block: @escaping (ObjectChange<Self>) -> Void) -> NotificationToken
  {
    ...
    let schema = schema
    return RLMObjectBaseAddNotificationBlock(rootObject, kps, queue)
    { object, names, oldValues, newValues, error in
      assert(error == nil) // error is no longer used
      guard let names, let newValues
      else
      {
        block(.deleted)
        return
      ...
      for i in 0 ..< newValues.count
      {
        let filter: (ProjectionProperty) -> Bool = { prop in
          ...
          guard let keyPaths, !keyPaths.isEmpty
          else
          {
            return true
          }
  ...
  ) async -> NotificationToken
  {
    ...
    { actor, obj in
      obj.observe(keyPaths: keyPaths, on: nil)
      { (change: ObjectChange<Self>) in
...
public extension ProjectionObservable
{
  ...
  func _observe<S>(_ keyPaths: [String]?, on queue: DispatchQueue?, _ subscriber: S) -> NotificationToken where S: Subscriber, S.Input == Self
  {
    ...
    { (change: ObjectChange<S.Input>) in
      switch change
      {
  ...
  {
    observe(keyPaths: [PartialKeyPath<Self>]()) { _ in _ = subscriber.receive() }
  }
}
...
final class AllocatedUnfairLock<Value>: @unchecked Sendable
{
...
@usableFromInline
@propertyWrapper
struct Unchecked<Wrapped>: @unchecked Sendable
{
...
private final class ProjectionSchemaCache: @unchecked Sendable
{
  ...
  fileprivate func schema<T: ProjectionObservable>(for obj: T) -> [ProjectionProperty]
  {
    ...
    for child in Mirror(reflecting: obj).children
    {
      ...
      guard !originPropertyLabel.isEmpty
      else
      {
