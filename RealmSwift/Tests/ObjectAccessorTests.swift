...
class ObjectAccessorTests: TestCase
{
  ...
  func testIntSizes()
  {
    ...
    try! realm.write
    {
      ...
      let testObject: () -> Void = {
        obj.objectSchema.properties.map(\.name).forEach { obj[$0] = 0 }
  ...
  func testLinkingObjectsDynamicGet()
  {
    ...
    guard let owners = dynamicOwners
    else
    {
    ...
    guard let firstOwner = (owners as? LinkingObjects<SwiftOwnerObject>)?.first
    else
    {
