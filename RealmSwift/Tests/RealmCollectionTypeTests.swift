  ...
  AggregateCollection.Element == CTTAggregateObject, AggregateCollection.Index == Int
{
  ...
  func realm() -> Realm
  {
    try! Realm(configuration: Config.config)
  }

  func getCollection(_: Realm) -> Collection
  {
    fatalError("Abstract method. Try running tests using Control-U.")
  }

  func getAggregateableCollectionInWrite(_: Realm) -> AggregateCollection
  {
    fatalError("Abstract method. Try running tests using Control-U.")
  }
  ...
  {
    let expected = Array(collection.map(\.stringCol))
  ...
  func testSetValueForKey()
  {
    ...
    let actual = Array(collection.map(\.stringCol))
  ...
  func testSortWithSwiftKeyPath()
  {
    ...
    nonOptionalSorted.enumerated().forEach
    { e in
  ...
  func testObserveOnActor() async throws
  {
    ...
    let token = await collection.observe(on: actor)
    { actor, changes in
      actor.check(changes)
    }
  ...
  func testCancelTaskForObservationInit() async throws
  {
    ...
    { @MainActor in
      ...
      let token = await collection.observe(on: CustomGlobalActor.shared)
      { _, _ in
  ...
  func testObserveOnActorWithStringKeyPath() async throws
  {
    let actor = TestActor()
    await actor.expect("initial notification")
    let token = await collection.observe(keyPaths: ["stringCol"], on: actor)
    { actor, changes in
  ...
  func testObserveKeyPath()
  {
    ...
    let token0 = collection.observe(keyPaths: ["stringCol"])
    { (changes: RealmCollectionChange) in
      switch changes
      {
        case let .initial(collection):
  ...
  {
    var ex = expectation(description: "initial notification")
    let token = collection.observe(keyPaths: ["linkCol.id"])
    { (changes: RealmCollectionChange) in
      switch changes
      {
        case let .initial(collection):
  ...
  {
    let ex = expectation(description: "initial notification")
    let token = collection.observe(keyPaths: ["linkCol"])
    { (changes: RealmCollectionChange) in
      switch changes
      {
        case let .initial(collection):
  ...
  func testObserveOnQueue()
  {
    ...
    let token = collection.observe(keyPaths: nil, on: queue)
    { (changes: RealmCollectionChange) in
      switch changes
      {
        case let .initial(collection):
...
class ResultsTests: RealmCollectionTests<Results<CTTNullableStringObjectWithLink>, Results<CTTAggregateObject>>
{
  ...
  func testNotificationBlockChangeIndices()
  {
    ...
    var calls = 0
    let token = collection.observe
    { (change: RealmCollectionChange) in
      switch change
      {
        case let .initial(results):
...
class ResultsFromTableTests: ResultsTests
{
  ...
  {
    realm.objects(CTTNullableStringObjectWithLink.self)
  }
...
class ResultsFromTableViewTests: ResultsTests
{
  override func getCollection(_ realm: Realm) -> Results<CTTNullableStringObjectWithLink>
  {
    realm.objects(CTTNullableStringObjectWithLink.self).filter("stringCol != ''")
  }
...
{
  override func getCollection(_: Realm) -> List<CTTNullableStringObjectWithLink>
  {
    CTTStringList(value: [[str1, str2]]).array
  }

  override func getAggregateableCollectionInWrite(_: Realm) -> List<CTTAggregateObject>
  {
    CTTAggregateObjectList(value: [makeAggregateableObjectsInWriteTransaction()]).list
  }
...
class MutableSetRealmCollectionTests: RealmCollectionTests<MutableSet<CTTNullableStringObjectWithLink>, MutableSet<CTTAggregateObject>>
{
  ...
  {
    let expected = Set(collection.map(\.stringCol))
...
{
  override func getCollection(_: Realm) -> MutableSet<CTTNullableStringObjectWithLink>
  {
    CTTStringSet(value: [[str1, str2]]).set
  }

  override func getAggregateableCollectionInWrite(_: Realm) -> MutableSet<CTTAggregateObject>
  {
    CTTAggregateObjectSet(value: [makeAggregateableObjectsInWriteTransaction()]).set
  }
