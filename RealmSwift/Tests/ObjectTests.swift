...
class ObjectTests: TestCase
{
  ...
  private func withMigrationObject(block: @escaping @Sendable (MigrationObject, Migration) -> Void)
  {
    ...
    autoreleasepool
    {
      let configuration = Realm.Configuration(schemaVersion: 1, migrationBlock: { migration, _ in
        migration.enumerateObjects(ofType: SwiftObject.className())
        { _, newObject in
          if let newObject
          {
  ...
  func testDeleteObservedObject() throws
  {
    ...
    let token0 = object0.observe
    { change in
      guard case .deleted = change
      else
      {
    ...
    let token1 = object1.observe(keyPaths: ["intCol"])
    { change in
      guard case .deleted = change
      else
      {
  ...
  func expectChange(_ name: String, _ old: (some Equatable)?, _ new: (some Equatable)?, _ inverted: Bool = false) -> ((ObjectChange<ObjectBase>) -> Void)
  {
  ...
  func testModifyUnobservedKeyPathLocally() throws
  {
    ...
    ex.isInverted = true
    let token = object.observe(keyPaths: ["boolCol"])
    { _ in
      ex.fulfill()
    }
    try object.realm!.write
  ...
  func testModifyMultipleObservedPartialKeyPathLocally() throws
  {
    ...
    var token = object.observe(keyPaths: [\SwiftObject.intCol, \SwiftObject.stringCol])
    { changes in
    ...
    token = object.observe(keyPaths: [\SwiftObject.intCol, \SwiftObject.stringCol])
    { changes in
      if case let .change(_, properties) = changes
      {
        XCTAssertEqual(properties.count, 1)
  ...
  func testModifyUnobservedPartialKeyPathLocally() throws
  {
    ...
    let ex = expectation(description: "no change")
    ex.isInverted = true
    let token = object.observe(keyPaths: [\SwiftObject.boolCol, \SwiftObject.stringCol])
    { _ in
      ex.fulfill()
    }
    try object.realm!.write
  ...
  func testListPropertyKeyPathNotifications()
  {
    ...
    ex.isInverted = true
    var token = company.observe(keyPaths: ["employees"])
    { _ in
      ex.fulfill()
    }
    try! realm.write
    ...
    ex.isInverted = true
    token = company.observe(keyPaths: ["employees.hired"])
    { _ in
      ex.fulfill()
    }
    try! realm.write
    ...
    ex = expectation(description: "no change notification")
    ex.isInverted = true
    token = company.observe(keyPaths: ["employees"])
    { _ in
      ex.fulfill()
    }
    try! realm.write
  ...
  func testLinkPropertyKeyPathNotifications3()
  {
    ...
    ex.isInverted = true
    let token = person.observe(keyPaths: ["dog"])
    { _ in
      ex.fulfill()
    }
    try! realm.write
  ...
  func testBacklinkPropertyKeyPathNotifications1()
  {
    ...
    let ex = expectation(description: "no change notification")
    ex.isInverted = true
    let token = dog.observe(keyPaths: ["owners"])
    { _ in
      ex.fulfill()
    }
    try! realm.write
  ...
  func testObserveOnDifferentQueue()
  {
    ...
    let token = object.observe(on: queue)
    { change in
  ...
  func testObserveKeyPathOnDifferentQueue()
  {
    ...
    let queue = DispatchQueue(label: "label")
    let sema = DispatchSemaphore(value: 0)
    let token = object.observe(keyPaths: ["intCol"], on: queue)
    { change in
      self.checkChange("intCol", 1, 2, change)
      sema.signal()
    }
  ...
  func testInvalidateObserverOnDifferentQueueBeforeRegistration()
  {
    ...
    let token1 = object.observe(on: queue)
    { _ in
      XCTFail("notification should not have fired")
    }
    let token2 = object.observe(on: queue)
    { _ in
      sema.signal()
    }
    token1.invalidate()
    ...
    -> @Sendable (isolated CustomGlobalActor, ObjectChange<T>) -> Void
  {
    ...
    { _, change in
      ...
      guard case let .change(object, props) = change
      else
      {
  ...
  func testObserveOnAlreadyCancelledTask() async throws
  {
    ...
    { @MainActor in
      ...
      let token = await obj.observe(on: MainActor.shared)
      { _, _ in
  ...
  func testCancelTaskWhileWaitingForInitial() async throws
  {
    ...
    await withTaskGroup(of: NotificationToken.self)
    { group in
      while active.value < 10
      {
        ...
        { @CustomGlobalActor in
          ...
          guard let realm = try? await Realm(actor: CustomGlobalActor.shared)
          else
          {
            waitingForRealm.withLock { $0 -= 1 }
