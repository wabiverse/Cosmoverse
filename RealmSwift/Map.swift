...
public final class Map<Key: _MapKey, Value: RealmCollectionValue>: RLMSwiftCollectionBase
{
  ...
  {
    _rlmCollection.realm.map { Realm($0) }
  }

  /// Indicates if the map can no longer be accessed.
  public var isInvalidated: Bool { _rlmCollection.isInvalidated }

  /// Returns all of the keys in this map.
  public var keys: [Key]
  {
    rlmDictionary.allKeys.map(staticBridgeCast)
  }

  /// Returns all of the values in this map.
  public var values: [Value]
  {
    rlmDictionary.allValues.map(staticBridgeCast)
  }
  ...
  init(objc rlmDictionary: RLMDictionary<AnyObject, AnyObject>)
  {
  ...
  @objc public var count: Int { Int(_rlmCollection.count) }
  ...
  /**
   ...
   - parameter combine: A closure that takes the current and new values for
               ...
               the final map.
   */
  public func merge(_ sequence: some Sequence<(key: Key, value: Value)>, uniquingKeysWith combine: (Value, Value) throws -> Value) rethrows
  {
    for (key, value) in sequence
    {
      let key = objcKey(from: key)
      var selectedValue: Value = if let existing = rlmDictionary[key]
      {
        try combine(staticBridgeCast(fromObjectiveC: existing), value)
      }
      else
      {
        value
  ...
  {
    rlmDictionary.object(forKey: key as AnyObject)
  }
  ...
  {
    rlmDictionary.value(forKey: key as AnyObject)
  ...
  {
    rlmDictionary.value(forKeyPath: keyPath)
      .map(dynamicBridgeCast)
  }
  ...
  {
    Results<Value>(rlmDictionary.objects(with: predicate))
  }
  ...
  {
    filter(isIncluded(Query()).predicate)
  }
  ...
  {
    var found = false
    rlmDictionary.enumerateKeysAndObjects
    { k, v, shouldStop in
  ...
  {
    sorted(byKeyPath: "self", ascending: ascending)
  }
  ...
  {
    Results<Value>(_rlmCollection.sortedResults(using: sortDescriptors.map(\.rlmSortDescriptorValue)))
  }
  ...
  {
    rlmDictionary.min(ofProperty: property).map(staticBridgeCast)
  }
  ...
  {
    rlmDictionary.max(ofProperty: property).map(staticBridgeCast)
  }
  ...
  {
    staticBridgeCast(fromObjectiveC: rlmDictionary.sum(ofProperty: property))
  }
  ...
  {
    rlmDictionary.average(ofProperty: property).map(staticBridgeCast)
  }
  ...
  {
    RLMDictionaryDescriptionWithMaxDepth("Map", rlmDictionary, depth)
  }

  var rlmDictionary: RLMDictionary<AnyObject, AnyObject>
  {
...
extension Map: Encodable where Key: Encodable, Value: Encodable
{
  public func encode(to encoder: Encoder) throws
  {
    ...
    try container.encode(reduce(into: [Key: Value]())
    { map, element in
...
extension Map: Sequence
{
  ...
  {
    RLMMapIterator(collection: rlmDictionary)
  }
}
...
public extension Map
{
  ...
  struct KeyValueSequence: Sequence
  {
    ...
    {
      RLMKeyValueIterator<Key, Value>(collection: map.rlmDictionary)
    }
  }

  /// Returns this Map as a sequence of `(key: Key, value: Value)`
  func asKeyValueSequence() -> KeyValueSequence
  {
    KeyValueSequence(self)
  }
}
