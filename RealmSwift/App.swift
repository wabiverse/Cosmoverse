...
public extension App
{
  ...
  {
    __updateBaseURL(url, completion: completion)
  }
  ...
  {
    __login(withCredential: ObjectiveCSupport.convert(object: credentials))
    { user, error in
      if let user
      {
  ...
  {
    __updateBaseURL(url, completion: { error in
      if let error
      {
  ...
  {
    future { self.login(credentials: credentials, $0) }
  }
...
@frozen public struct AppSubscription: Subscription
{
  ...
  init(token: RLMAppSubscriptionToken)
  {
    self.token = token
  }
...
/// :nodoc:
@available(macOS 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
public struct AppPublisher: Publisher, @unchecked Sendable
{ // DispatchQueue
  ...
  init(_ app: App, scheduler: some Scheduler)
  {
    self.app = app
...
extension App: ObservableObject
{
  ...
  {
    AppPublisher(self, scheduler: DispatchQueue.main)
  }
}

@available(macOS 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
func promisify(_ fn: @escaping (@escaping @Sendable (Error?) -> Void) -> Void) -> Future<Void, Error>
{
  future
  { promise in
    fn
...
public extension APIKeyAuth
{
  ...
  {
    future { self.createAPIKey(named: named, completion: $0) }
  }
  ...
  {
    future { self.fetchAPIKey(objectId, $0) }
  }
