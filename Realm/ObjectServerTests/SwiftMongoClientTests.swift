 ...
 * ---------------------------------------------------------------- */
  ...
  class SwiftMongoClientTests: SwiftSyncTestCase
  {
    ...
    func performWatchWithMatchFilterTest(_ queue: DispatchQueue?) throws
    {
      ...
      let changeStream: ChangeStream = if let queue
      {
        collection.watch(matchFilter: filter, delegate: watchTestUtility, queue: queue)
      }
      else
      {
        collection.watch(matchFilter: filter, delegate: watchTestUtility)
      }
      watchTestUtility.waitForOpen()
    ...
    func performWatchWithFilterIdsTest(_ queue: DispatchQueue?) throws
    {
      ...
      {
        collection.watch(filterIds: [objectIds[0]], delegate: watchTestUtility)
      }
      watchTestUtility.waitForOpen()
