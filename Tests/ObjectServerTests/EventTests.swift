...
class SwiftEventTests: SwiftSyncTestCase
{
  ...
  override func tearDown()
  {
    if let user
    {
      while !user.allSessions.isEmpty
      {
  ...
  {
    let realm = try Realm.asyncOpen(configuration: configuration()).await(self)
  ...
  func testCustomEventRepresentation() throws
  {
    ...
    scope1.commit().awaitFailure(self)
    { error in
    ...
    scope2.commit().awaitFailure(self)
    { error in
  ...
  func testReadEvents() throws
  {
    ...
    func assertEvent(_ activity: String, _ value: [NSDictionary]..., line: UInt = #line)
    {
      ...
      for (expected, actual) in zip(value, filtered.map(\.parsedData))
      {
