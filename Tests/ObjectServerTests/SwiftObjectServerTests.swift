 ...
 * ---------------------------------------------------------------- */
  ...
  class SwiftObjectServerTests: SwiftSyncTestCase
  {
    ...
    func testUpdateBaseUrl()
    {
      ...
      app.updateBaseUrl(to: "http://localhost:8080")
      { result in
        switch result
        {
          case .success:
      ...
      app.updateBaseUrl(to: "http://localhost:7070/")
      { result in
        switch result
        {
          case .success:
    ...
    func testSwiftAddObjectsWithNilPartitionValue() throws
    {
      ...
      let app = try app(id: RealmServer.shared.createApp(types: [SwiftPerson.self]))
    ...
    func testConnectionState() throws
    {
      ...
      func wait(forState desiredState: SyncSession.ConnectionState)
      {
        ...
        let token = session.observe(\SyncSession.connectionState, options: .initial)
        { s, _ in
    ...
    func testStreamingDownloadNotifier() throws
    {
      ...
      let token = session.addProgressNotification(for: .download, mode: .reportIndefinitely)
      { p in
    ...
    func testStreamingUploadNotifier() throws
    {
      ...
      let token = session.addProgressNotification(for: .upload, mode: .reportIndefinitely)
      { p in
        DispatchQueue.main.async
        { @MainActor in
          if let progress
    ...
    func testStreamingNotifierInvalidate() throws
    {
      ...
      let tokenDownload = session.addProgressNotification(for: .download, mode: .reportIndefinitely)
      { _ in
        downloadCount.wrappedValue += 1
      }
      let tokenUpload = session.addProgressNotification(for: .upload, mode: .reportIndefinitely)
      { _ in
    ...
    func testDNSError() throws
    {
      ...
      Realm.asyncOpen(configuration: config).awaitFailure(self, timeout: 40)
      { error in
    ...
    func testTLSError() throws
    {
      ...
      Realm.asyncOpen(configuration: config).awaitFailure(self)
      { error in
    ...
    func testInvalidCredentials() throws
    {
      ...
      app.login(credentials: credentials2)
      { result in
        guard case .failure = result
        else
        {
    ...
    {
      AppConfiguration(baseURL: "http://localhost:9090")
    }
    ...
    func testWriteCopyFailBeforeSynced() throws
    {
      ...
      XCTAssertThrowsError(try realm.writeCopy(configuration: user2Config))
      { error in
    ...
    func testServerSchemaValidationWithCustomColumnNames() throws
    {
      ...
      RealmServer.shared.retrieveSchemaProperties(appId, className: className)
      { result in
        switch result
        {
          case let .failure(error):
