...
extension Object: _RealmCollectionValueInsideOptional
{
  ...
  @objc override open dynamic var isInvalidated: Bool { super.isInvalidated }
   ...
   */
  @objc open class func primaryKey() -> String? { nil }
...
extension Object: ThreadConfined
{
  /**
   ...
   - see: `Object.freeze()`
   */
  public var isFrozen: Bool { realm?.isFrozen ?? false }
  ...
  {
    guard let realm else { throwRealmException("Unmanaged objects cannot be frozen.") }
  ...
  {
    guard let realm else { throwRealmException("Unmanaged objects cannot be thawed.") }
...
@frozen public enum ObjectChange<T>
{
  ...
  init(object: T?, names: [String]?, oldValues: [Any]?, newValues: [Any]?)
  {
    guard let names, let newValues, let object
    else
    {
      self = .deleted
      return
    ...
    self = .change(object, (0 ..< newValues.count).map
    { i in
...
func dynamicSet(object: ObjectBase, key: String, value: Any?)
{
  let bridgedValue: Any? = if let v1 = value, let v2 = v1 as AnyObject as? _ObjcBridgeable
  {
    v2._rlmObjcValue
  }
  else
