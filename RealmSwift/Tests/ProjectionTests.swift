...
class ProjectionTests: TestCase
{
  ...
  func expectPropertyChange(_ obj: AllTypesPrimitiveProjection,
                            _ keyPath: KeyPath<AllTypesPrimitiveProjection, some Any>,
                            ...
                            _ callback: @escaping (AllTypesPrimitiveProjection, Any?, Any?) -> Void) -> (XCTestExpectation, NotificationToken)
  {
    ...
    let token = obj.observe(keyPaths: [keyPath])
    { changes in
      ex.fulfill()
      guard case let .change(object, properties) = changes
      else
      {
  ...
  )
  {
    ...
    let (ex, token) = expectPropertyChange(obj, keyPath, name)
  ...
  func testObserveKeyPath()
  {
    ...
    let token = johnProjection.observe(keyPaths: ["lastName"], on: nil)
    { _ in
      ex.fulfill()
    }
    dispatchSyncNewThread
  ...
  func testObserveNestedProjection()
  {
    ...
    let token = johnProjection.observe(keyPaths: [\PersonProjection.mobile])
    { changes in
      if case let .change(_, propertyChange) = changes
      {
    ...
    let token2 = johnProjection.observe
    { changes in
      if case let .change(_, propertyChange) = changes
      {
    ...
    let token3 = johnProjection.observe
    { changes in
      if case let .change(_, propertyChange) = changes
      {
  ...
  {
    guard let change = observeChange(obj, key, block)
    else
    {
  ...
  func testObserveOnActor() async throws
  {
    ...
    let block = { @Sendable (_: isolated CustomGlobalActor, change: ObjectChange<SimpleProjection>) in
      guard case let .change(_, properties) = change
      else
      {
  ...
  func testObserveComputedChange() throws
  {
    ...
    let token = johnProjection.observe(keyPaths: [\PersonProjection.lastNameCaps])
    { chg in
      if case let .change(_, change) = chg
      {
        ex.fulfill()
        guard let value = change.first
        else
        {
