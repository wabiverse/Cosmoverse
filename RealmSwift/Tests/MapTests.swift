...
class MapTests: TestCase
{
  ...
  func testPopulateEmptyMap()
  {
    ...
    for object in map
    {
      XCTAssertTrue(!object.key.description.utf16.isEmpty, "Object should have description")
      XCTAssertTrue(!object.value!.description.utf16.isEmpty, "Object should have description")
      count += 1
    }
  ...
  func testNotificationSentInitially()
  {
    ...
    let token = mapObj.observe(on: queue)
    { change in
      switch change
      {
  ...
  func testObserveKeyPath()
  {
    ...
    let token = mapObj.swiftObjectMap.observe(keyPaths: ["intCol"])
    { (changes: RealmMapChange) in
      switch changes
      {
        case let .initial(map):
          XCTAssertEqual(map.count, 2)
  ...
  func testObserveKeyPathBacklink()
  {
    ...
    var ex = expectation(description: "initial notification")
    let token = mapObj.dogMap.observe(keyPaths: ["owners.name"])
    { (changes: RealmMapChange) in
      switch changes
      {
        case let .initial(map):
  ...
  @MainActor func testObserveOnActor() async throws
  {
    ...
    let token = await map.observe(keyPaths: ["intCol"], on: MainActor.shared)
    { _, change in
      switch change
      {
        case .initial:
...
class MapNewlyCreatedTests: MapTests
{
  ...
  override func createEmbeddedMap() -> Map<String, EmbeddedTreeObject1?>
  {
    realm.create(EmbeddedParentObject.self).map
  }
}
